...
void ScanChain( TChain* chain, TString outputname, int nEvents = -1 )
{
    ...
    bool isDoubleMuon = outputname.Contains( "doublemuon" );
    bool isEWK = ( outputname.Contains( "WJets" ) || outputname.Contains( "DY" ) );
    bool isQCD = outputname.Contains( "QCD" );
    ...
    int nptbins = 7;
    int netabins = 3;
    double ptbins[8] = {10., 15., 20., 25., 30., 35., 50., 70.};
...
void fill( TasUtil::AutoHist& hists, TString suffix, float& evt_mt, float& evt_met, float& weight )
{
    ...
    if ( abs( id() ) == 11 )
    {
        ...
        hists.fill( p4().pt(), "histo_pt_" + suffix + "_el", weight, 20, 0, 100);
    ...
    else if ( abs( id() ) == 13 )
    {
        ...
        hists.fill( p4().pt(), "histo_pt_" + suffix + "_mu", weight, 20, 0, 100 );
