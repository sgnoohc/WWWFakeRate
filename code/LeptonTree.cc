...
void LeptonTree::Init(TTree *tree) {
  ...
  instantLumi_branch = tree->GetBranch("instantLumi");
  if (instantLumi_branch) instantLumi_branch->SetAddress(&instantLumi_);
...
void LeptonTree::LoadAllBranches() {
  ...
  if (instantLumi_branch != 0) instantLumi();
  if (rho_branch != 0) rho();
...
const float &LeptonTree::instantLumi() {
  if (not instantLumi_isLoaded) {
    if (instantLumi_branch != 0) {
      instantLumi_branch->GetEntry(index);
    } else {
      printf("branch instantLumi_branch does not exist!\n");
      exit(1);
    }
    instantLumi_isLoaded = true;
  }
  return instantLumi_;
}
...
const float &instantLumi() { return lepton_tree.instantLumi(); }
